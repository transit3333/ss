...
function buildRailGraph() {
    mapState.railGraph = Pathfinding.buildRailGraph(mapState.railWays, mapState.railNodes);
}
...
function zoomAround(clientX, clientY, delta) {
    ...
    mapState.center = unproject(centerWorld, nextZoom);
    renderMap();
    scheduleStationLoad();
}
...
function initPointerEvents() {
    ...
    mapCanvas.addEventListener('pointermove', (event) => {
        ...
        if (pointer.dragged) {
            ...
            mapState.center = unproject(centerWorld, mapState.zoom);
            renderMap();
        }
    });
